...
* Load the best model available, degrading gracefully:
  1. the fine-tuned DistilBERT checkpoint in ``MODEL_DIR``
  2. a pretrained sentiment model from the Hub (``FALLBACK_MODEL``)
  3. the TF-IDF baseline saved by ``src.train_baseline``
  4. a transparent lexicon heuristic, so the API *always* answers
...
The model is loaded once at process start and reused. Torch inference runs
under ``torch.inference_mode`` with gradients disabled and threads pinned, so
a CPU container stays predictable under concurrent requests.
"""
